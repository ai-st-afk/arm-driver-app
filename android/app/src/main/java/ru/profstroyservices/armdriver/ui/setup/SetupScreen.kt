    ...
    Scaffold { innerPadding ->
        Column(
            ...
            Button(
                ...
                Text("Сохранить и запросить разнарядку", style = MaterialTheme.typography.labelLarge)
            }

            when (val state = uiState.loadState) {
                is AssignmentLoadState.Idle -> Unit
                is AssignmentLoadState.Loading -> CircularProgressIndicator()
                is AssignmentLoadState.Success -> Text(
                    text = "Разнарядка получена",
            ...
            uiState.savedDriverId?.let {
                Button(
                    ...
                    Text("Перейти к разнарядке", style = MaterialTheme.typography.labelLarge)
                }
            }
        }
    }
