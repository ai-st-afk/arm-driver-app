...
5. **Формат доставки фото документа в 1С.** Контракт `<Событие>` вложений не предусматривает. Нужно узнать у 1С-команды: отдельный HTTP-метод для файлов на их стороне, base64 в отдельном XML-сообщении, или что-то ещё. До ответа фото лежат только у нас в техническом delivery-cache (см. раздел 13, п. 2).
