...
2. ~~Фото подписанной накладной~~ — решено 2026-09-18: нужно, снимается с камеры (не из галереи) на шаге «Разгрузился». Реализовано на стороне gateway/Android как технический delivery-cache (`POST /api/mobile/documents`, см. docs/backend-api.md); хранится 90 дней / 7 дней после подтверждённой доставки в 1С. Открыт только следующий пункт — сама доставка в 1С.
...
3. Тестовый эндпоинт для стыковки до боевого включения.
   **Не закрыт:** `POST .../hs/prtr_driver/photo` на тестовом сервере отвечает
   `404` — судя по всему, метод ещё не опубликован (запрос сформирован строго
   по их спеке, проверено логами шлюза).
4. Где физически хранятся токены FCM устройств — в справочнике 1С или в шлюзе.
6. **Передача разнарядки другому водителю.** Приходит ли прежнему водителю та же
   разнарядка со `<Статус>Отменена</Статус>`, когда диспетчер переназначает её
   после часа без приёма? Шлюз шлёт уведомление «разнарядку передали другому»
   именно по приходу отменённой разнарядки — если 1С её не присылает, водитель
   останется с разнарядкой, которую у него уже забрали.
5. ~~Формат доставки фото документа в 1С~~ — закрыто 2026-09-21: отдельный `POST .../hs/prtr_driver/photo`, бинарное тело, метаданные в заголовках (`X-Photo-Id`/`X-Assignment-Id`/`X-Trip-Id`), идемпотентность по `X-Photo-Id`, лимит 5 МБ. Реализовано в шлюзе, см. docs/backend-api.md. Координаты места съёмки 1С предлагала опционально — отказались, GPS вне объёма проекта.
...
Раздел 3 выше — бизнес-архитектура (кто с кем обменивается данными и зачем).
Это — как оно физически разложено на хосте. Общая схема всей сети (домены,
соседние проекты) — `~/go/src/infra/ARCHITECTURE.md`.

```mermaid
flowchart LR
    Traefik["Traefik\n(infra, сеть edge)"] -->|"arm-driver.profstroy-services.ru"| Gateway

    subgraph "docker-compose (этот репозиторий)"
        Gateway["gateway\nGo, stateless"]
        Cache[("volume: arm_driver_gateway_data\nFCM-токены + последние XML разнарядок")]
        Gateway --- Cache
    end

    Gateway <-->|"XML, X-Auth-Token"| OneC["1С:ERP заказчика"]
    Gateway -->|push| FCM["Firebase Cloud Messaging"]
    Android["Android-приложение водителя"] <-->|"JSON, HTTPS"| Gateway
```

`gateway` — единственный контейнер, без собственной БД: `arm_driver_gateway_data`
не бизнес-хранилище, а технический delivery-cache (см. раздел 13, п. 2) —
переживает пересборку образа, но не источник истины (тот — 1С). Не
принимает подключений напрямую от 1С/Android без Traefik — весь HTTP-вход
через `edge`, порт на хост не публикуется.
