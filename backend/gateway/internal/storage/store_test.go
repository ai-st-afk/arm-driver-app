...
func TestCleanupOldDocumentsRemovesExpiredOnly(t *testing.T) {
	...
	fresh, err := store.SaveDocument("trip-1", "driver-1", "assignment-1", ".jpg", []byte("fresh"))
	if err != nil {
		t.Fatalf("save fresh document: %v", err)
	}
	stalePending, err := store.SaveDocument("trip-2", "driver-1", "assignment-1", ".jpg", []byte("stale-pending"))
	if err != nil {
		t.Fatalf("save stale pending document: %v", err)
	}
	staleDelivered, err := store.SaveDocument("trip-3", "driver-1", "assignment-1", ".jpg", []byte("stale-delivered"))
	if err != nil {
